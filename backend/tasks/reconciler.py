...
def reconcile_single_campaign(campaign_id: int, db) -> dict:
    ...
    the appropriate tasks. Tasks use distributed stage locks to prevent duplicate execution.
    """
    dispatched = []
...
def reconcile_campaigns_task() -> dict:
    ...
    try:
        ...
        logger.info("Reconciler sweep completed: %d campaigns had pending work dispatched", len(results))
        return {
            "status": "success",
